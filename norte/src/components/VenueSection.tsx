...
type Props = {
  settings: Pick<SettingsSite, "venueName" | "venueAddress" | "venueCity" | "venueMapUrl" | "venueLat" | "venueLng">;
};

function buildEmbedUrl(lat: string, lng: string): string {
  return `https://www.google.com/maps?q=${lat},${lng}&z=15&output=embed`;
}

function buildDirectionsUrl(lat: string, lng: string): string {
  return `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`;
}

const VenueSection = ({ settings }: Props) => {
  const hasCoords = !!(settings.venueLat && settings.venueLng);
  const embedUrl = hasCoords
    ? buildEmbedUrl(settings.venueLat!, settings.venueLng!)
    : settings.venueMapUrl || null;
  const directionsUrl = settings.venueMapUrl || (hasCoords
    ? buildDirectionsUrl(settings.venueLat!, settings.venueLng!)
    : null);
      ...
      <div className="container-main">
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-center">
          <ScrollReveal>
            ...
            {directionsUrl ? (
              <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="btn-outline-primary text-sm">Como Chegar</a>
            ) : (
          ...
          <ScrollReveal delay={100}>
            <div className="aspect-[4/3] rounded-2xl bg-muted flex items-center justify-center border border-border overflow-hidden">
              {embedUrl ? (
                <iframe
                  src={embedUrl}
